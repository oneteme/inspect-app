...
export class StatsApiComponent implements OnInit, OnDestroy {
    ...
    API_REQUEST = (name: string, env: string, start: Date, end: Date, advancedParams: FilterMap) => {
        ...
        return {
            ...
            dependentsTable: { observable: this._statsService.getSessionApi({ 'column': "count:count,count_succes:countSucces,count_error_client:countErrClient,count_error_server:countErrServer,request2.api_name:name", "request.id": "out.parent", "out.id": "request2.id", 'request.api_name': name, 'request.start.ge': start.toISOString(), 'request.start.lt': end.toISOString(), 'request2.start.ge': start.toISOString(), 'request2.start.lt': end.toISOString(), 'environement': env, 'request2.api_name.not': 'null', 'order': 'count.desc', ...advancedParams }) },
            dependenciesTable: { observable: this._statsService.getSessionApi({ 'column': "count:count,count_succes:countSucces,count_error_client:countErrClient,count_error_server:countErrServer,request.api_name:name", "request.id": "out.parent", "out.id": "request2.id", 'request2.api_name': name, 'request2.start.ge': start.toISOString(), 'request2.start.lt': end.toISOString(), 'request.start.ge': start.toISOString(), 'request.start.lt': end.toISOString(), 'environement': env, 'request.api_name.not': 'null', 'order': 'count.desc', ...advancedParams }) },
