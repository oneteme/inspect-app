...
export class StatsAppComponent implements OnInit, OnDestroy {
    ...
    APP_REQUEST = (name: string, env: string, start: Date, end: Date, advancedParams: FilterMap) => {
        ...
        return {
            ...
            repartitionUserBar: {
                observable: this._statsService.getRestSession({ 'column': `count:count,rest_session.start.${groupedBy}:date,rest_session.start.year:year,instance.user:user`, 'rest_session.instance_env': 'instance.id', 'instance.app_name': name, 'rest_session.start.ge': start.toISOString(), 'rest_session.start.lt': end.toISOString(), 'instance.environement': env, 'instance.user.not': 'null', 'order': `rest_session.start.year.asc,rest_session.start.${groupedBy}.asc`, ...advancedParams }).pipe(map((r: any[]) => {
