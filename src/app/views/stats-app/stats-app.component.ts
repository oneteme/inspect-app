...
export class StatsAppComponent implements OnInit, OnDestroy {
    ...
    APP_REQUEST = (name: string, env: string, start: Date, end: Date, advancedParams: FilterMap) => {
        ...
        return {
            ...
            repartitionRequestByPeriodLine: { observable: this._statsService.getSessionRest({ 'column': "count:count,count_error_server:countErrorServer,count_slowest:countSlowest,start.date:date", 'app_name': name, 'start.ge': new Date(now.getFullYear(), now.getMonth(), now.getDate() - 6).toISOString(), 'start.lt': now.toISOString(), 'environement': env, 'order': 'start.date.asc', ...advancedParams }) },
            repartitionUserPolar: { observable: this._statsService.getSessionRest({ 'column': "count:count,user:user", 'app_name': name, 'start.ge': start.toISOString(), 'start.lt': end.toISOString(), 'environement': env, 'user.not': 'null', 'order': 'count.desc', ...advancedParams }).pipe(map((r: any[]) => r.slice(0, 5))) },
            repartitionUserBar: {
                observable: this._statsService.getSessionRest({ 'column': `count:count,start.${groupedBy}:date,start.year:year,user:user`, 'app_name': name, 'start.ge': start.toISOString(), 'start.lt': end.toISOString(), 'environement': env, 'user.not': 'null', 'order': `start.year.asc,start.${groupedBy}.asc`, ...advancedParams }).pipe(map((r: any[]) => {
            ...
            dependenciesTable: { observable: this._statsService.getSessionRest({ 'column': "count:count,count_succes:countSucces,count_error_client:countErrClient,count_error_server:countErrServer,apisession.app_name:name", "apisession.id": "apirequest.parent", "apirequest.id": "apisession2.id", 'apisession2.app_name': name, 'apisession2.start.ge': start.toISOString(), 'apisession2.start.lt': end.toISOString(), 'apisession.start.ge': start.toISOString(), 'apisession.start.lt': end.toISOString(), 'environement': env, 'apisession.app_name.not': 'null', 'order': 'count.desc', ...advancedParams }) },
            exceptionsTable: { observable: this._statsService.getSessionRest({ 'column': 'count,err_type,err_msg', 'err.group': '', "status.ge": 500, "environement": env, "app_name": name, 'start.ge': start.toISOString(), 'start.lt': end.toISOString(), 'order': 'count.desc', ...advancedParams })}
        };
    };
