...
export class StatsDatabaseComponent implements OnInit {
  ...
  setServerFormValue() {
    ...
    this._statsService.getSessionRest({ 'column.distinct': 'dbquery.db&dbquery.parent=apisession.id&order=dbquery.db.asc', 'apisession.environement': this.env }).pipe(catchError(error => of(error)))
      .subscribe({
        next: (res: { db: string }[]) => {
