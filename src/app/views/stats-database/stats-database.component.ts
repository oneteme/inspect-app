...
export class StatsDatabaseComponent implements OnInit {
  ...
  DB_REQUEST = (db: string, env: string, start: Date, end: Date) => {
    ...
    return {
      ...
      exceptions: { observable: this._statsService.getSessionApi({ 'column': 'count,dbaction.err_type.coalesce(null),dbaction.err_msg.coalesce(null)', 'dbaction.err_type.not': 'null', 'dbaction.err_msg.not': 'null', 'dbaction.parent': 'dbquery.id', 'dbquery.parent': 'apisession.id', 'order': 'count.desc', "apisession.environement": env, "dbquery.db": db, 'apisession.start.ge': start.toISOString(), 'apisession.start.lt': end.toISOString(), 'dbquery.start.ge': start.toISOString(), 'dbquery.start.lt': end.toISOString() }).pipe(map((d: any) => d.slice(0, 5))) },
