...
export class StatisticRequestSmtpComponent {
  ...
  performanceRepartitionChange(event){
    switch(event.type) {
      case 'slice':
        this.getCustom(this.$performanceRepartitionSlice, this.getSliceColumns(event, SMTP_REPARTITION_PERFORMANCE_JQUERY_CONFIG), null);
        break;
      default:
        if(!event.config.selectedSerie){
          event.config.selectedSerie = "elapsedtime";
        }
        this.getCustom(this.$performanceRepartition, this.getColumns(event, SMTP_REPARTITION_PERFORMANCE_JQUERY_CONFIG), event.config.selectedGroup);
        break;
    }
  }
  ...
  getCustom(arr: { data: any[], loading: boolean, stats: any},
    ...
    return this._smtpRequestService.getCustom(
      columns, {
