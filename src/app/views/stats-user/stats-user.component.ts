...
export class StatsUserComponent implements OnInit, OnDestroy {
    ...
    USER_REQUEST = (name: string, env: string, start: Date, end: Date, advancedParams: FilterMap) => {
        ...
        return {
            ...
            exceptionsTable: { observable: this._statsService.getSessionApi({ 'column': 'count,err_type,err_msg', 'err.group': '', "status.ge": 500, "environement": env, 'user': name, 'start.ge': start.toISOString(), 'start.lt': end.toISOString(), 'order': 'count.desc', ...advancedParams })},
