...
import {ChartConfig, FTP_PERFORMANCE_CHART_CONFIG, FTP_STATUS_CHART_CONFIG} from "../../kpi.config";
...
export class FtpComponent implements OnInit {
  ...
  getCustom(event: {eventType: 'default' | 'filter', chartConfig: ChartConfig, filteredTasks?: any[]},
    ...
    if(event.eventType === 'default') {
      arr.loading = true;
      arr.data = [];
      this._ftpRequestService.getCustom({series: arr.chartConfig.series.items, indicator: actualIndicator, group: actualGroup, stack: actualStack, filter: actualFilter}, {env: this.params.env, start: this.params.period.start, end: this.params.period.end, hosts: this.params.hosts, filters: event.filteredTasks})
      ...
      if(actualFilter) {
        this._ftpRequestService.getFilters(actualFilter, {env: this.params.env, start: this.params.period.start, end: this.params.period.end, hosts: this.params.hosts}).subscribe({
          next: (res: any[]) => {
            slice.data = res;
          }
        });
