...
export class SmtpComponent implements OnInit {
  ...
  getCustom(event: {eventType: 'default' | 'filter', chartConfig: ChartConfig, filteredTasks?: any[]},
    ...
    if(event.eventType === 'default') {
      arr.loading = true;
      arr.data = [];
      this._smtpRequestService.getCustom({series: arr.chartConfig.series.items, indicator: actualIndicator, group: actualGroup, stack: actualStack, filter: actualFilter}, {env: this.params.env, start: this.params.period.start, end: this.params.period.end, hosts: this.params.hosts, filters: event.filteredTasks})
  ...
  getCommands() {
    ...
    this._smtpRequestService.getSmtp(args).pipe(finalize(() => this.$commandRepartition.loading = false)).subscribe({
      next: (res: any[]) => {
  ...
  getDependencies() {
    ...
    this._smtpRequestService.getSmtp(args).pipe(finalize(() => this.$dependencyRepartition.loading = false)).subscribe({
      next: (res: any[]) => {
  ...
  getUser() {
    ...
    this._smtpRequestService.getSmtp(args).pipe(finalize(() => this.$userRepartition.loading = false)).subscribe({
      next: (res: any[]) => {
