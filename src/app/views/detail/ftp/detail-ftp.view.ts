    ...
    createTimeline() {
        ...
        let items = this.request.actions.map((a: FtpRequestStage, i:number) => {
            ...
            let item:any = {
                ...
                title: `<span>${this.pipe.transform(new Date(a.start * 1000), 'HH:mm:ss.SSS')} - ${this.pipe.transform(new Date(end), 'HH:mm:ss.SSS')}</span> (${this.durationPipe.transform({start: a.start, end: end/1000})})<br>
