...
export class DetailFtpView implements OnInit, OnDestroy {
  ...
  getRequest() {
    ...
    forkJoin([
      this._traceService.getFtpRequest(this.params.idFtp),
      this._traceService.getFtpRequestStages(this.params.idFtp)
    ]).pipe(
  ...
  createTimeline() {
    ...
    if (this.dataArray.length > 50) {
      ...
      groups = getDataForRange(this.stages, this.timelineStart, this.timelineEnd).map((a: FtpRequestStage, i: number) => ({
        id: i,
        content: a?.name,
        treeLevel: 2
      }))
  ...
  onTimelineCreate(timeline: Timeline) {
    timeline.on('rangechanged', (props) => {
      ...
      })), props.start.getTime(), props.end.getTime()).map((a: FtpRequestStage, i: number) => ({
        id: `${d[i + 1].group}`,
        content: a?.name,
        treeLevel: 2
      }))
