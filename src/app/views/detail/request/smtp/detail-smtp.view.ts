...
export class DetailSmtpView implements OnInit, OnDestroy {
    ...
    createTimeline() {
        ...
        if(this.dataArray.length > 50){
            ...
            groups = getDataForRange(this.stages, this.timelineStart , this.timelineEnd).map((a:MailRequestStage, i:number) => ({ id: i, content: a?.name,treeLevel: 2}))
        }else {
    ...
    onTimelineCreate(timeline: Timeline) {
        timeline.on('rangechanged', (props)=>{
            ...
            let groups:any[]= getDataForRange(this.stages.map(s=>({...s, start:Math.trunc(s.start*1000), end: s.end ? Math.trunc(s.end*1000 ) : INFINITY })), props.start.getTime() , props.end.getTime()).map((a: MailRequestStage, i:number) => ({ id: `${d[i+1].group}`, content: a?.name, treeLevel: 2}))
            groups.splice(0,0,{id:'parent', content: this.request.threadName,treeLevel: 1, nestedGroups:groups.map(g=>(g.id))})
