...
export class InstanceService {
  ...
  getInstanceInfo(filters: { env: string, id: string }): Observable<InstanceEnvironment> {
    return this.getInstance<any>({
      'column': `id,app_name:name,version,collector,start:instant,os,re,address,user,branch,configuration,type,end,resource,hash`,
      'id.varchar': `"${filters.id}"`
    }).pipe(map(res => { return res.map(r => ({...r, configuration: r.configuration?.value ? JSON.parse(r.configuration?.value) : null, resource: r.resource?.value ? JSON.parse(r.resource?.value) : null}))[0] }));
  }
  ...
  getInstancesPeriodsByAppName(filters: { env: string,appName: string, address: string | undefined}): Observable<{
    ...
    let args: any = {
      'column': 'id,start,end.coalesce(2040-12-31T00:00:00.000Z):end,version,address',
      'environement': filters.env,
      'app_name': `"${filters.appName}"`,
      'order': 'start.asc'
    }
